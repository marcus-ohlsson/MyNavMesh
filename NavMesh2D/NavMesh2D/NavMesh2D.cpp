...
void AddOps(std::vector<sf::RectangleShape>& ops, World& world, sf::RenderWindow& window)
{
	...
	sf::Vector2f targetPos(
		gridPos.X * World::CellSize,
		gridPos.Y * World::CellSize
	);

	for (int i = 0; i < ops.size(); ++i) 
	{
		int gridIndex = gridPos.Y * world.WorldSize.X + gridPos.X;

		if (world.Grid[gridIndex].IsBlocked)
		{
			if (ops[i].getPosition() == targetPos)
			{
				ops.erase(ops.begin() + i);
				world.Grid[gridIndex].IsBlocked = false;
				return;
			}
		}
	}
